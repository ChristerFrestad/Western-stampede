...
function injectLonghorns(strips: SymbolId[][], count: number): SymbolId[][] {
 ...
 left--;
 } else if (strip[idx] !== SymbolId.LONGHORN && strip[idx] !== SymbolId.SCATTER) {
 // occasionally overwrite mid symbols
 if (guard % 3 === 0) {
 strip[idx] = SymbolId.LONGHORN;
 left--;
 }
...
export class SpinEngine {
 ...
 let mode: GameMode = freeSession && freeSession.remaining > 0 ? 'FREE' : 'BASE';
 let debitAmount = 0;
 ...
 // First spin of buy is a free spin (no extra debit)
 mode = 'FREE';
 ...
 // Stampede roll — rarer during free games (already elevated EV)
 const baseStampede = this.math.features.stampedeChance + (freeSession?.stampedeBoost ?? 0);
 ...
 stops.push(await this.rng.nextInt(len));
 }
 ...
 // Stampede guarantee: force at least one LONGHORN on every reel
 if (stampede) {
 grid = ensureFiveOfAKind(grid, SymbolId.LONGHORN);
 }

 // Force free games for testing
 ...
 // Cap max win
 const maxWin = Math.floor(bet * this.math.maxWinX);
 if (totalWin > maxWin) {
 const scale = maxWin / totalWin;
 wins = wins.map((w) => ({
 ...w,
 amount: Math.floor(w.amount * scale),
 }));
 totalWin = wins.reduce((a, w) => a + w.amount, 0);
 }

 // Supercoin wheel during free games
 let supercoin: SupercoinResult | null = null;
 ...
 const room = Math.max(0, this.math.features.supercoinCap - freeSession.longhornInjected);
 const awarded = Math.min(wheelValue, room);
 freeSession.longhornInjected += awarded;
 supercoin = {
 ...
 // Buy enhanced/premium: supercoin on entry once (before first spin eval already done —
 // apply if buyTier requested supercoin and LONGHORN still 0)
 if (
 input.buyTier &&
 freeSession &&
 freeSession.longhornInjected === 0
 ) {
 const opt = this.math.buyOptions.find((b) => b.tier === input.buyTier);
 if (opt?.supercoinOnEntry && !supercoin) {
 const values = this.math.features.supercoinWheelValues;
 const idx = await this.rng.nextInt(values.length);
 const wheelValue = values[idx]!;
 freeSession.longhornInjected += wheelValue;
 supercoin = {
 awardedLonghorns: wheelValue,
 totalLonghornsInjected: freeSession.longhornInjected,
 wheelValue,
 };
 }
 ...
 freeSession.remaining += freeGamesAwarded;
 freeSession.totalAwarded += freeGamesAwarded;
 }
 ...
 mode: stampede ? 'STAMPEDE' : inFree ? 'FREE' : buyTier ? 'BUY' : 'BASE',
 bet,
 grid,
 heights,
 stops,
...
function ensureFiveOfAKind(grid: SymbolId[][], symbol: SymbolId): SymbolId[][] {
 ...
 const copy = [...reel];
 // Replace center-ish cell with symbol
